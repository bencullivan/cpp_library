...
template <typename T> struct point {
	...
	db dist_to_seg(const point &a, const point &b) {
		if ((*this - a).angle(b - a) * 2 <= PI && (*this - b).angle(a - b) * 2 <= PI) {
	...
	// order
	bool inside_convex(const std::vector<point> &polygon, bool clockwise, bool include_edges = 1) {
		for (size_t i = 0; i < polygon.size(); i++) {
			point a = polygon[i + 1 == polygon.size() ? 0 : i + 1] - polygon[i];
			point b = *this - polygon[i];
			T v = a.cross(b);
			if ((v > 0 && clockwise) || (v < 0 && !clockwise) || (v == 0 && !include_edges)) {
				return 0;
			}
		}
		return 1;
...
void transform(std::vector<point<T>> &a, bool include_collinear = false) {
	...
	for (int i = 1; i < (int)a.size(); i++) {
		if (i == (int)a.size() - 1 || cw(a[i] - p1, mid, include_collinear)) {
			while (up.size() >= 2 && !cw(up[up.size() - 1] - up[up.size() - 2], a[i] - up[up.size() - 2], include_collinear)) {
				up.pop_back();
			}
			up.push_back(a[i]);
		}
		if (i == (int)a.size() - 1 || ccw(a[i] - p1, mid, include_collinear)) {
			while (down.size() >= 2 && !ccw(down[down.size() - 1] - down[down.size() - 2], a[i] - down[down.size() - 2], include_collinear)) {
				down.pop_back();
			}
			down.push_back(a[i]);
		}
...
template <typename T>
std::vector<int> indices(const std::vector<point<T>> &a, bool include_collinear = false) {
	if (a.size() == 0) return {};
	...
	for (int i = 1; i < (int)a.size(); i++) {
		if (i == (int)a.size() - 1 || cw(a[id[i]] - p1, mid, include_collinear)) {
			while (up.size() >= 2 && !cw(a[up[up.size() - 1]] - a[up[up.size() - 2]], a[id[i]] - a[up[up.size() - 2]], include_collinear)) {
				up.pop_back();
			}
			up.push_back(id[i]);
		}
		if (i == (int)a.size() - 1 || ccw(a[id[i]] - p1, mid, include_collinear)) {
			while (down.size() >= 2 && !ccw(a[down[down.size() - 1]] - a[down[down.size() - 2]], a[id[i]] - a[down[down.size() - 2]], include_collinear)) {
				down.pop_back();
			}
			down.push_back(id[i]);
		}
	...
	id.clear();
	for (int i = 0; i < (int)up.size(); i++) id.push_back(up[i]);
	for (int i = down.size() - 2; i > 0; i--) id.push_back(down[i]);
	return id;
}

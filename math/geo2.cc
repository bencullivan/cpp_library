 ...
 * License: CC0
 * Source: Basic math
 * Description: Returns the signed distance between point p and the line
 * containing points a and b. Positive value on left side and negative on right
 * as seen from a towards b. a==b gives nan. P is supposed to be Point<T> or
 * Point3D<T> where T is e.g. double or long long. It uses products in
 * intermediate steps so watch out for overflow if using int or long long. Using
 * Point3D will always give a non-negative distance. For Point3D, call .dist on
 * the result of the cross product. Status: tested
 */
 ...
 * is returned and if infinitely many exists \{-1, (0,0)\} is returned. The
 * wrong position will be returned if P is Point<ll> and the intersection point
 * does not have integer coordinates. Products of three coordinates are used in
 * intermediate steps so watch out for overflow if using int or ll. Usage: auto
 * res = lineInter(s1,e1,s2,e2); if (res.first == 1) cout << "intersection point
 * at " << res.second << endl; Status: stress-tested, and tested through
 * half-plane tests
 */
 ...
 * Status: stress-tested
 */
template <class P> P lineProj(P a, P b, P p, bool refl = false) {
  P v = b - a;
 ...
 * Returns false in case of no intersection. Status: stress-tested
 */
 ...
 * Description: The circumcirle of a triangle is the circle intersecting all
 * three vertices. ccRadius returns the radius of the circle going through
 * points A, B and C and ccCenter returns the center of the same circle. Status:
 * tested
 */
#define P Point<double>
 ...
 * Source:
 * Description:
 * Returns the shortest distance between point p and the line segment from point
 * s to e. Usage: Point<double> a, b(2,2), p(1,1); bool onSegment =
 * segDist(a,b,p) < 1e-10; Status: tested
 */
#define P Point<double>
 ...
 * order. (Epsilon checks may optionally be added to sideOf/sgn, but shouldn't
 * be needed.) Time: $O(N^2)$, where $N$ is the total number of points Status:
 * stress-tested, Submitted on ECNA 2017 Problem A
 */
#define P Point<double>
double polyUnion(vector<vector<P>> &poly) {
  static auto ratio = [](P a, P b) -> double {
 ...
 * License: CC0
 * Source:
 * Description: Returns a vector with the vertices of a polygon with everything
 * to the left of the line going from s to e cut away. Usage: vector<P> p = ...;
 ...
 * have no collinear points. lineHull(line, poly) returns a pair describing the
 ...
 * on side $(i, i+1)$. The points are returned in the same order as the line
 * hits the polygon. extrVertex returns the point of a hull with the max
 * projection onto a line. Time: O(\log n) Status: stress-tested
 */

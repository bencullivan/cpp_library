 ...
 * gcd(a,b) Returns gcd(a,b) Time: O(log(min(a,b)) Source:
 ...
 * Chicken McNugget Theorem: for any positive coprime integers a,b the largest
 * integer that cannot be written in the form ax + by for nonnegative x,y is ab
 * - a - b
 *
 * Time: O(log(min(abs(a),abs(b)))
 * Source: https://cp-algorithms.com/algebra/linear-diophantine-equation.html
 */
bool linear_diophantine(long long a, long long b, long long c, long long &x,
 ...
 * Source: https://cp-algorithms.com/algebra/linear-diophantine-equation.html
 */
long long all_linear_diophantine(long long a, long long b, long long c,
  ...
  auto shift_solution = [](long long &x, long long &y, long long a, long long b,
                           long long cnt) {
    x += cnt * b;
    y -= cnt * a;
  };

...
template<typename T, typename U = typename enable_if<!is_same<T, string>::value, typename T::value_type>::type>
...
void _dba(T* a, int d) {
	...
	cerr << "}";
	if (d == 0) cerr << RESET << endl;
}
template<typename T>
void _dba(const vector<T>& a, int d) {
	cerr << "\n{";
	string sep = "";
	for (int i = 0; i < _dm[d]; i++) {
		cerr << sep, sep = ", ";
		if (d == _mx - 1) cerr << a[i];
		else _dba(a[i], d + 1);
	}

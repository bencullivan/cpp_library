...
struct SkipList {
    ...
    struct node {
        ...
        T data;
        node *pr, *nx, *lo, *hi;
        node() : ih(1), data(T()), pr(nullptr), nx(nullptr), lo(nullptr), hi(nullptr) {}
        node(const T& t) : ih(0), data(t), pr(nullptr), nx(nullptr), lo(nullptr), hi(nullptr) {}
    };
    node* root;
    int h, s;
    SkipList()
        : sl_rng(std::chrono::high_resolution_clock::now().time_since_epoch().count())

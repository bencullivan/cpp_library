...
struct segtree {
    ...
    segtree(std::vector<node> &input)
        : data(4 * input.size()), len(input.size()) {
    ...
    void upd(int i, int tl, int tr, int x, const node &v) {
        ...
        if (x <= tm)
            ...
            upd(i * 2 + 1, tm + 1, tr, x, v);
        // update the current node
        data[i] = combine(data[i * 2], data[i * 2 + 1]);
    }
    // sets the element at index x to v
    void upd(int x, const node &v) { upd(1, 0, len - 1, x, v); }
    ...
    node get(int i, int tl, int tr, int l, int r) {
        ...
        int tm = (tl + tr) / 2;
        return combine(get(i * 2, tl, tm, l, r),
                       get(i * 2 + 1, tm + 1, tr, l, r));
    }
    // queries the segment [l, r]
    node get(int l, int r) { return get(1, 0, len - 1, l, r); }
    // --------------------------------------
    // CHANGE
    struct node {
        long long v = 0;
    };
    node combine(const node &a, const node &b) { return {a.v + b.v}; }
    // --------------------------------------
}; // segtree

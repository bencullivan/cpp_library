// Lazy Segtree inspired by the implementation from the AtCoder Library (except
// this one is recursive lol) Range updates and queries in O(log(n))
...
struct lazy_segtree {
  ...
  lazy_segtree(int _len)
      : segs(ceil_pow(_len) * 2 + 1), lazy(ceil_pow(_len) * 2 + 1), len(_len) {}
  lazy_segtree(const vector<S> &input)
      : segs(ceil_pow(input.size()) * 2 + 1),
        lazy(ceil_pow(input.size()) * 2 + 1), len(input.size()) {

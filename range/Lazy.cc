...
struct LazySegtree {
	...
	LazySegtree(int _len) : segs(ceil_pow(_len) * 2 + 1), lazy(ceil_pow(_len) * 2 + 1), len(_len) {}
	LazySegtree(const vector<S>& input) : segs(ceil_pow(input.size()) * 2 + 1), lazy(ceil_pow(input.size()) * 2 + 1), len(input.size()) {

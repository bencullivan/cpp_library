...
struct LazyS {
    ...
    LazyS(int _len) : data(4 * _len), len(_len) {}
 
    LazyS(vector<Node>& input) : data(4 * input.size()), len(input.size()) {
        build_LazyS(1, 0, len - 1, input);
    }
 
    // builds the Segtree using the nodes in the input array
    void build_LazyS(int i, int tl, int tr, vector<Node>& input) {
        if (tl == tr) {
        ...
        int tm = (tl + tr) >> 1;
        build_LazyS(i << 1, tl, tm, input);
        build_LazyS(i << 1 | 1, tm + 1, tr, input);
...
// --------------------------------------
    ...
    Node duplicate(const Lazy& a, int c) {

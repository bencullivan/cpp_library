...
// }
// F composition(F l, F r) { // composition of two lazy updates (l is applied
// after r)
// }
...
template <bool VALS_EDGES, typename S, S (*op)(S, S), S (*e)(), typename F,
...
struct HeavyLightDecomposition {
  ...
  HeavyLightDecomposition(int _root, const vector<vector<int>> &tr,
    ...
    st = atcoder::lazy_segtree<S, op, e, F, mapping, composition, id>(
        ordered_vals);
  }
  ...
  void upd_vertical(int u, int ancestor, const F &x,
                    bool dont_update_ancestor) {
    for (; head[u] != head[ancestor]; u = par[head[u]])
      st.apply(pos[head[u]], pos[u] + 1, x);
  ...
  void upd(int u, int v, const F &x) {
    ...
    if (u == top)
      upd_vertical(v, top, x, VALS_EDGES);
    else if (v == top)
...
F_HLD composition_hld(
    F_HLD l,
    F_HLD r) { // composition of two lazy updates (l is applied after r)
}
S_HLD e_hld() { return S_HLD(); }  // the identity segment
F_HLD id_hld() { return F_HLD(); } // the identity update
template <bool VALS_EDGES>
using HLD = HeavyLightDecomposition<VALS_EDGES, S_HLD, op_hld, e_hld, F_HLD,
                                    mapping_hld, composition_hld, id_hld>;

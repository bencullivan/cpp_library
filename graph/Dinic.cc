...
struct Dinic {
	vector<vector<int>>
			adj; // adj[i] is the list of indices of the out-edges of the ith vertex
	...
	vector<int>
			ptr; // ptr[i] points to the next unvisited neighbor of the ith vertex
	queue<int> q;
	int source;
	int sink;
	...
	// adds the edge with the given capacity and the reverse edge to the residual
	// network
	...
	// attempts to push 'push_flow' units of flow from vertex u toward the sink
	// i.e. up a level returns the number of units that were pushed

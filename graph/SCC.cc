 ...
 * Finds all strongly connected components of a directed graph with N vertices
 * and M edges. Time: O(N+M) Source: CS31 Lecture Notes - Deeparnab Chakrabarty,
 * folklore
 */
struct SCC {
  ...
  std::vector<int> which_scc; // which_scc[i] is the number of the scc that
                              // vertex i belongs to
  ...
  // builds a single scc
  void dfs(int current_vertex, std::vector<int> &current_scc,
  ...
  // builds the DAG of sccs as well as labels each vertex with the scc it
  // belongs to builds G_scc and sccs in topological order of sccs
  void get_sccs() {
    order.clear();
